 ...
 * Handles initial loading from location state, data cache, and remote fetching.
 */
export function useChatParticipant({
    ...
    useEffect(() => {
        ...
        if (!isGroupChat && otherUserId && otherUserId !== 'group') {
            useUserStore.getState().fetchUserIfNeeded(otherUserId)
                .then(user => {
                    if (isMounted && user) {
                        setOtherUser(prev => {
                            const chat = allChats?.find(c => c.metadata?.otherUserId === otherUserId);
                            const name = chat?.name || user.name || 'Unknown User';
                            return { ...user, ...prev, name, contact_name: chat?.name };
                        });
                    }
                })
                .catch(() => {
                    if (isMounted) setOtherUser({ id: otherUserId, name: 'Unknown User', avatar: null });
                });
        }
