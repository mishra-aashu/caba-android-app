...
export const initializeFileSystem = async () => {
  ...
  try {
    try {
      const stat = await safePluginCall(
...
export const saveChatsToDevice = async (allChats) => {
  ...
  try {
    ...
    await safePluginCall(
      () => import('@capacitor/filesystem'),
      (mod, { Directory, Encoding }) => mod.Filesystem.writeFile({
...
export const loadChatsFromDevice = async () => {
  if (!isNativeWithPlugins()) return [];
  try {
    const contents = await safePluginCall(
      () => import('@capacitor/filesystem'),
      (mod, { Directory, Encoding }) => mod.Filesystem.readFile({
        path: 'CaBa/chats.json',
        directory: Directory.Data,
        encoding: Encoding.UTF8,
      })
...
export const saveImageToDevice = async (photoUrl, messageId) => {
  ...
  try {
    ...
    const savedFile = await safePluginCall(
      () => import('@capacitor/filesystem'),
...
export const clearAllCachedData = async () => {
  ...
  try {
    ...
    await safePluginCall(
      () => import('@capacitor/filesystem'),
      (mod, { Directory }) => mod.Filesystem.rmdir({
        path: 'CaBa',
        directory: Directory.Data,
        recursive: true,
      })
